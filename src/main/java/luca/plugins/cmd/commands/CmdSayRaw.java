...
public class CmdSayRaw extends CustomCommand {

    public CmdSayRaw() {
        super("sayraw", "lcommands.sayraw", "raw");
        setUsage("<message...>");
        setDescription("Broadcasts a raw colored message.");
    }

    public void execute(CommandSender sender, CommandArgs args) {
        if (args.length() > 0) {
            Bukkit.broadcastMessage(StringUtils.convertColors(args.toString()));
        } else {
