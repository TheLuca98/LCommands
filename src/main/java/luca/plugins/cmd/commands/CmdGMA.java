...
public class CmdGMA extends CustomCommand {
    ...
    public void execute(CommandSender sender, CommandArgs args) {
        if (args.length() > 0) {
            CmdGameMode.INSTANCE.execute(sender, new CommandArgs(new String[] {GameMode.ADVENTURE.name(), args.get(0)}));
        } else {
            CmdGameMode.INSTANCE.execute(sender, new CommandArgs(new String[] {GameMode.ADVENTURE.name()}));
        }
    }
